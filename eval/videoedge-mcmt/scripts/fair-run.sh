...
set -uo pipefail
REPO=/home/anrg/dsf; NET="${1:?need net label}"; REPS="${REPS:-6}"
OUT=/tmp/fair-results.csv
[ -f "$OUT" ] || echo "net,cell,rep,system,phase,makespan_s,wall_s" > "$OUT"
CELLS=("30 jpg" "60 jpg" "120 jpg" "120 png")
...
run_wl(){ local cell=$1 r=$2 tpl=$3 s=$(date +%s) out run p ms
  ...
  for i in $(seq 1 120); do p=$(kubectl -n dsf-system get odag "$run" -o jsonpath='{.status.phase}' 2>/dev/null); case "$p" in Succeeded|Failed) break;; esac; sleep 5; done
  ms=$(kubectl -n dsf-system get odag "$run" -o jsonpath='{.status.makespan}' 2>/dev/null); local w=$(( $(date +%s)-s ))
...
for spec in "${CELLS[@]}"; do
  read d fmt <<<"$spec"; cell="d${d}-${fmt}"; WL="vemcmt-n4-d${d}-${fmt}-spread"; AR="vemcmt-n4-d${d}-${fmt}-spread-argo"
